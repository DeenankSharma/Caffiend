...
interface CoffeeConsumptionHistory {
  [key: string]: CoffeeRecord;
}
...
interface CoffeeStats {
  daily_caffeine: string;
  daily_cost: string;
  average_coffees: string;
  total_cost: string;
}
...
export function calculateCurrentCaffeineLevel(historyData: Record<string, CoffeeEntry>): string {
...
export function getTopThreeCoffees(historyData: Record<string, CoffeeEntry>): CoffeeCount[] {
...
export function calculateCoffeeStats(coffeeConsumptionHistory: Record<string, CoffeeEntry>): CoffeeStats {
